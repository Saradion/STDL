		...
		if (_other instanceof SequenceTypeImpl) {
			...
			if (this.types.size() == _local.types.size()) {
				...
				while (i1.hasNext() && i2.hasNext() && _result) {
					_result = i1.next().equalsTo(i2.next());
				}
				return _result;
			} else {
				return false;
			...
			if (_other instanceof ArrayTypeImpl) {
				...
				while (_iter.hasNext() && _result) {
					_result = _iter.next().equalsTo(_element);
				}
				return _result;
			} else {
				return _other instanceof RecordTypeImpl && this.equalsTo(((RecordTypeImpl) _other).erase());
			}
		}
	}
	...
	public boolean compatibleWith(Type _other) {
		if (_other instanceof SequenceTypeImpl) {
			...
			if (this.types.size() == _local.types.size()) {
				...
				boolean _result = true;
				while (i1.hasNext() && i2.hasNext() && _result) {
					_result = i1.next().compatibleWith(i2.next());
				}
				return _result;
			} else {
				return false;
			...
			if (_other instanceof ArrayTypeImpl) {
				...
				return _other instanceof RecordTypeImpl && this.compatibleWith(((RecordTypeImpl) _other).erase());
			}
		}
	}

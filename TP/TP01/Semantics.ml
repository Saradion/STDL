...
ruleName env name = 
    ...
    | (Found value) -> 
        match value with
        | (FrozenValue (expr, fenv)) -> value_of_expr expr fenv
        | _ -> value
...
ruleLet env ident bvalue bin = 
    let var = (value_of_expr bvalue env) in
        match var with
        | ErrorValue _ -> var
        | _ -> let env = (ident, var)::env in (value_of_expr bin env)
(* ========================================================*)
and 
